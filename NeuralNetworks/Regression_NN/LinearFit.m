...
subplot(1,2,1)
scatter(Y1,Yn,8,'filled') % what is subplotted
%% Function Objective 1.a: linear fit
coeffs = polyfit(Y1,Yn, 1); %polyfit looks for best fit coefficients of data
% % Get fitted values
...
P_V_table=P_v;
 ...
 hold on;
 txt = {['Figure 4.2.', num2str(fc)];[strip(Yn_name)];['m = ',num2str(coeffs(1)), '  b = ', num2str(coeffs(2))];['R = ',num2str(sqrt(R_sq)), '  P = ',num2str(P_v),' R^2= ',num2str(R_sq)]};
 xlabel(Y1_name)
 ylabel(Yn_name)
